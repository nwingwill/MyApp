...
class ItemDetailVC: UIViewController {
    ...
    @IBOutlet weak var itemDetailCV: UICollectionView!
    ...
    @IBAction func goBackBtn(_ sender: Any) {
        ...
        let navigationOPtion = "goToHomeSG"
    ...
    func configureDetailView() {
        ...
        if idItem != nil || idItem != "" {
            ...
            self.itemDetailCV.register(UINib(nibName: "MainElementCVViewCell", bundle: nil), forCellWithReuseIdentifier: "mainElement")
            
            self.itemDetailCV.layer.cornerRadius = 5.0
            self.itemDetailCV.layer.masksToBounds = true
            
            self.itemDetailCV.dataSource = self
            self.itemDetailCV.delegate = self
        } else {
            alert.showAlert(title: "Failure", message: "No id Selected \(idItem ?? "No value")", alertType: .failure)
            subTitleLbl.text = "\(idItem ?? "No Value...!!!")"
        }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if navigationOPtion == "goToHomeSG"  {
